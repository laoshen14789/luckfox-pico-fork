...
static const struct regval g_sc530ai_1440x800_100fps_reg_table[] =
{
    {0x0103, 0x01},
    {0x0100, 0x00},
    {0x36e9, 0x80},
...
static void sc530ai_modify_fps_info(struct sc530ai *sc5330ai)
{
	...
	struct i2c_client *client = sc5330ai->client;

	sc5330ai->cur_fps.denominator = mode->max_fps.denominator * mode->vts_def /
					sc5330ai->cur_vts;
	dev_info(&client->dev, "max fps=%d vts_def=%d set fps=%d\n", mode->max_fps.denominator, mode->vts_def, sc5330ai->cur_fps.denominator);
}
...
static int sc530ai_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		dev_info(&client->dev, "width:%d height:%d\n", sc530ai->cur_mode->width, sc530ai->cur_mode->height);
		dev_info(&client->dev, "set vblank 0x%x vts=%d\n", ctrl->val, vts);
		break;
	case V4L2_CID_HFLIP:
